...
export const stringifyChannelName = (
...
export const stringifyGuildName = (
...
export const stringifyOptionsData = (
  data: readonly CommandInteractionOption[],
): string =>
  data
    .map((option): string => {
      const optionName = option.name;
      let optionValue: string | number | boolean | null | undefined =
        option.value;
      ...
      switch (option.type) {
        ...
        case ApplicationCommandOptionType.Subcommand:
          optionValue = option.options
            ? stringifyOptionsData(option.options)
            : null;
          break;
      }
...
export const getBareNumberFormatter = (
  interaction: Pick<CommandInteraction, 'locale'>,
) => {
  let numberFormatter: BareNumberFormatter | undefined =
    numberFormatterCache[interaction.locale];
  if (!numberFormatter) {
    ...
    try {
      ...
      console.error(
        `Failed to create number formatter with locale ${interaction.locale}`,
      );
      console.error(e);
    }
