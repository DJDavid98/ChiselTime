...
export const getTotalServerCount = async (client: Client): Promise<number> => {
  ...
  const shardGuildCounts = (await client.shard.fetchClientValues(
    'guilds.cache.size',
  )) as number[];
...
export const getTotalUserCount = async (client: Client): Promise<number> => {
  ...
  const shardGuilds = (await client.shard.fetchClientValues(
    'guilds.cache',
  )) as GuildManager['cache'][];
  ...
  return shardGuilds.reduce((acc, guild) => {
    return (
      acc +
      guild.reduce(
        (guildMemberCount, cachedGuild) =>
          guildMemberCount + cachedGuild.memberCount,
        0,
      )
    );
  }, 0);
